...
//        viewsql.append("SELECT * FROM \(schema).\(tablename) WHERE redeemed = 0; ")
        ...
        viewsql.append("client_transaction_id text, terminal_id int, disputed int, disputedby text, customer_code text, redeemed int, serial_number text) ")
        ...
        viewsql.append("            SELECT ct.id, ct.created, ct.amount, ct.total_amount, ct.client_location, ct.client_transaction_id, ct.terminal_id, ct.disputed, ct.disputedby, ct.customer_code, ct.redeemed, tm.serial_number ")
        ...
        viewsql.append("            SELECT cth.id, cth.created, cth.amount, cth.total_amount, cth.client_location, cth.client_transaction_id, cth.terminal_id, cth.disputed, cth.disputedby, cth.customer_code, cth.redeemed, tm.serial_number ")
...
//        viewsql.append("$$ LANGUAGE plpgsql; ")
        ...
        var viewsql = "CREATE OR REPLACE FUNCTION us.gettransactionreport("
        viewsql.append("    fromdate bigint, ")
        viewsql.append("    todate bigint, ")
        viewsql.append("    retailerid integer, ")
        viewsql.append("    terminalid integer DEFAULT 0, ")
        viewsql.append("    retaileruserid integer DEFAULT 0, ")
        viewsql.append("    offsetby integer DEFAULT 0, ")
        viewsql.append("    limitby integer DEFAULT 200) ")
        viewsql.append("RETURNS TABLE(id integer, created integer, amount numeric, total_amount numeric, client_location text, client_transaction_id text, terminal_id integer, disputed integer, disputedby text, customer_code text, redeemed integer) ")
        viewsql.append("LANGUAGE 'plpgsql' ")
        
        viewsql.append("COST 100 ")
        viewsql.append("VOLATILE ")
        viewsql.append("ROWS 1000 ")
        viewsql.append("AS $BODY$ ")
