...
public class USBucketAccountStatus: PostgresStORM {
    ...
    override open func to(_ this: StORMRow) {
        ...
        if let data = this.data.usBucketAccountStatusDic.change_date {
            change_date = data
        }
        
        if let data = this.data.usBucketAccountStatusDic.change_time {
            change_time = data
        }
        
        if let data = this.data.usBucketAccountStatusDic.account_number {
            account_number = data
        }
        
        if let data = this.data.usBucketAccountStatusDic.value_original {
            value_original = data
        }
        
        if let data = this.data.usBucketAccountStatusDic.value_new {
            value_new = data
        }
        
        if let data = this.data.usBucketAccountStatusDic.note {
            note = data
        }

    }
    
    func rows() -> [USBucketAccountStatus] {
        var rows = [USBucketAccountStatus]()
        for i in 0..<self.results.rows.count {
            let row = USBucketAccountStatus()
    ...
    func asDictionary() -> [String: Any] {
        ...
        if self.change_date.isNotNil {
            dictionary.usBucketAccountStatusDic.change_date = self.change_date
        }
        
        if self.change_time.isNotNil {
            dictionary.usBucketAccountStatusDic.change_time = self.change_time
        }
        
        if self.account_number.isNotNil {
            dictionary.usBucketAccountStatusDic.account_number = self.account_number
        }
        
        if self.value_original.isNotNil {
            dictionary.usBucketAccountStatusDic.value_original = self.value_original
        }
        
        if self.value_new.isNotNil {
            dictionary.usBucketAccountStatusDic.value_new = self.value_new
        }

        if self.note.isNotNil {
            dictionary.usBucketAccountStatusDic.note = self.note
        }
    ...
    func compare(targetItem: USBucketAccountStatus)-> Bool {
