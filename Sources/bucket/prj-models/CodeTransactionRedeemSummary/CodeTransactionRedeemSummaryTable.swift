...
final class CodeTransactionRedeemSummaryTable {
    ...
    static let sharedInstance = CodeTransactionRedeemSummaryTable()
    let tbl = CodeTransactionRedeemSummary()
    ...
    func create() {
        ...
        var tr = try! config.sqlRows(thesql, params: ["public.table_\(tbl.table())"])
        if tr.count > 0 {
            ...
            thesql = "INSERT INTO config(name,val) VALUES('public.table_\(tbl.table())','1.00')"
    ...
    private func update(currentlevel: Double) {
        ...
        print("UPDATE public.\(tbl.table().capitalized).  Current Level \(currentlevel), Required Level: \(tablelevel)")
    ...
    private func table()-> String {
        ...
        // common
        createsql.append("( ")
        createsql.append("id integer NOT NULL DEFAULT nextval('public.\(tbl.table())_id_seq'::regclass), ")
