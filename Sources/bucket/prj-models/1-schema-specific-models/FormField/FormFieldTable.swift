...
final class FormFieldTable {
    ...
    static let sharedInstance = FormFieldTable()
    let tbl = FormField()
    
    let tablelevel = 1.00
    
    //MARK:-
    //MARK: Form Field table
    func create() {
        
        for i in PRJCountries.list  {
            createFormField((i.uppercased()))
        }
    }

    //MARK:-
    //MARK: formfield table
    private func createFormField(_ schemaIn:String? = "public") {
    ...
    private func table(_ schemaIn:String? = "public")-> String {
        ...
        createsql.append("length int NOT NULL DEFAULT 0, ")
        createsql.append("is_required bool NOT NULL DEFAULT false, ")
        createsql.append("needs_confirmation bool NOT NULL DEFAULT false, ")
