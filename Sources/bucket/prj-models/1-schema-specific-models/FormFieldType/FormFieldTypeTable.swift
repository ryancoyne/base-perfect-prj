...
final class FormFieldTypeTable {
    ...
    static let sharedInstance = FormFieldTypeTable()
    let tbl = FormFieldType()
    
    let tablelevel = 1.00
    
    //MARK:-
    //MARK: Form Field Type table
    func create() {
        
        for i in PRJCountries.list  {
            createFormFieldType((i.uppercased()))
        }
    }

    //MARK:-
    //MARK: Form Field Type table
    private func createFormFieldType(_ schemaIn:String? = "public") {
