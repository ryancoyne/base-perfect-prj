...
public class FormFieldType: PostgresStORM {
    ...
    //MARK: Table name
    override public func table() -> String { return "form_field_type" }
    ...
    override open func to(_ this: StORMRow) {
        ...
        if let data = this.data.formFieldTypeDic.name {
            name = data
        }
        
    }
    
    func rows() -> [FormFieldType] {
        var rows = [FormFieldType]()
        for i in 0..<self.results.rows.count {
            let row = FormFieldType()
    ...
    func asDictionary() -> [String: Any] {
        ...
        if self.name.isNotNil {
            dictionary.formFieldTypeDic.name = self.name
        }
    ...
    func compare(targetItem: FormFieldType)-> Bool {
