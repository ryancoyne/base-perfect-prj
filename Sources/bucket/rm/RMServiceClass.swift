...
final class RMServiceClass {
    
    private init() {
        RMConfig.runSetup()
        displayTitle = RMConfig.getVal("title", "")
        displaySubTitle = RMConfig.getVal("subtitle","")
        displayLogo = RMConfig.getVal("logo","/assets/images/no-logo.png")
        displayLogoSrcSet = RMConfig.getVal("logosrcset","/assets/images/no-logo.png 1x, /assets/images/no-logo.svg 2x")
        let si = RMConfig.getVal("sysinit", "0")
        systemInit = si.toBool() ?? false
        print("RMServiceClass.sharedInstance.swift: Complete init")
    ...
    static let sharedInstance = RMServiceClass()
...
//    }()
    ...
    static func getFilename()->String {
        ...
        if RMServiceClass.doesFileExist(filename: newfilename) {
    ...
    static func movePicture(picturename: File, todirectory: Dir, newfilename: String = "")-> Bool {
        ...
        RMServiceClass.doesDirectoryExist(todirectory)
