...
DROP FUNCTION us.gettransactionreporttotals(bigint,bigint,integer,integer,integer);

CREATE OR REPLACE FUNCTION us.getTransactionReportTotals(fromDate bigint, toDate bigint, retailerId int, terminalId int=0, retailerUserId int=0, eventId int=0, OUT total_count bigint, OUT total_value numeric, OUT total_sales numeric, OUT total_refund_value numeric, OUT total_refund_sales numeric)
AS
$$
...
FOR var_r IN (SELECT ct.redeemed, ct.amount, ct.total_amount, ct.refunded FROM us.code_transaction_view_deleted_no AS ct WHERE (ct.created BETWEEN fromDate AND toDate) AND (ct.retailer_id = retailerId) AND (ct.terminal_id = terminalId) AND (ct.retailer_user_id = retailerUserId) AND (ct.event_id = eventId) UNION ALL SELECT cth.redeemed, cth.amount, cth.total_amount, cth.refunded FROM us.code_transaction_history_view_deleted_no AS cth WHERE (cth.created BETWEEN fromDate AND toDate) AND (cth.retailer_id = retailerId) AND (cth.terminal_id = terminalId) AND (cth.retailer_user_id = retailerUserId) AND (cth.event_id = eventId))
LOOP
total_count := total_count + 1;
IF var_r.refunded = 0 THEN
total_value := total_value + var_r.amount;
...
FOR var_r IN (SELECT ct.redeemed, ct.amount, ct.total_amount, ct.refunded FROM us.code_transaction_view_deleted_no AS ct WHERE (ct.created BETWEEN fromDate AND toDate) AND (ct.retailer_id = retailerId) AND (ct.terminal_id = terminalId) AND  (ct.event_id = eventId) UNION ALL SELECT cth.redeemed, cth.amount, cth.total_amount, cth.refunded FROM us.code_transaction_history_view_deleted_no AS cth WHERE (cth.created BETWEEN fromDate AND toDate) AND (cth.retailer_id = retailerId) AND (cth.terminal_id = terminalId) AND (cth.event_id = eventId))
LOOP
total_count := total_count + 1;
IF var_r.refunded = 0 THEN
total_value := total_value + var_r.amount;
...
FOR var_r IN (SELECT ct.redeemed, ct.amount, ct.total_amount, ct.refunded FROM us.code_transaction_view_deleted_no AS ct WHERE (ct.created BETWEEN fromDate AND toDate) AND (ct.retailer_id = retailerId) AND (ct.retailer_user_id = retailerUserId) AND  (ct.event_id = eventId) UNION ALL SELECT cth.redeemed, cth.amount, cth.total_amount, cth.refunded FROM us.code_transaction_history_view_deleted_no AS cth WHERE (cth.created BETWEEN fromDate AND toDate) AND (cth.retailer_id = retailerId) AND (cth.retailer_user_id = retailerUserId) AND (cth.event_id = eventId))
LOOP
total_count := total_count + 1;
IF var_r.refunded = 0 THEN
total_value := total_value + var_r.amount;
...
FOR var_r IN (SELECT ct.redeemed, ct.amount, ct.total_amount, ct.refunded FROM us.code_transaction_view_deleted_no AS ct WHERE (ct.created BETWEEN fromDate AND toDate) AND (ct.retailer_id = retailerId) AND (ct.event_id = eventId) UNION ALL SELECT cth.redeemed, cth.amount, cth.total_amount, cth.refunded FROM us.code_transaction_history_view_deleted_no AS cth WHERE (cth.created BETWEEN fromDate AND toDate) AND (cth.retailer_id = retailerId) AND (cth.event_id = eventId))
LOOP
total_count := total_count + 1;
IF var_r.refunded = 0 THEN
total_value := total_value + var_r.amount;
